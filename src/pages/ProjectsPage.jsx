...
function ProjectsPage() {
    ...
    <>
        ...
        <section className="min-h-screen flex dark:text-white pt-5">
            <div className="fixed top-0 w-screen bg-opacity-95 bg-[#0000005d] backdrop-blur-sm z-50 flex justify-center md:flex-col md:top-[70px] md:w-[20%] md:h-[calc(100vh-70px)] lg:w-[15%]">
                {projectType.map(item => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        className={({isActive}) => `${isActive && 'bg-gray-700'} text-white hover:bg-[#421156] duration-300 p-1 px-3 rounded-3xl mx-2 m-2 md:m-1`
                    }
                    >{item.name}</NavLink>
                ))}
            </div>
